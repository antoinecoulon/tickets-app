...
export default function CreateTicketPage() {
    const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<TicketFormInputs>()
    ...
    const onSubmit = async (data: TicketFormInputs) => {
        ...
        try {
            ...
            setSubmitError("Erreur lors de la création d'un ticket, veuillez réessayer.")
        }
    }
    ...
    return (
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          ...
          <select className="w-full border rounded p-2" {...register('priorite', { required: true })}>
            <option value="">-- Sélectionner --</option>
            <option value="basse">Basse</option>
            <option value="moyenne">Moyenne</option>
            <option value="haute">Haute</option>
          </select>
          {errors.priorite && <p className="text-red-600 text-sm">La priorité est requise</p>}
        </div>
